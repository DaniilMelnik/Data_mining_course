...
class GbBNewsParse(GbBlogParse):
    ...
    def parse_post(self, url, soup):
        ...
        comments = requests.get(
            urljoin(url, f"/api/v2/comments?commentable_type=Post&commentable_id={post_id}&order=desc")).json()
        data = {
            'post_data': {
                "url": url,
                'id': post_id,
            ...
            'author_data': {
                "url": urljoin(url, author.parent["href"]),
                "name": author.text
            },
            "tags_data": [
                {"name": tag.text,
...
if __name__ == "__main__":
    ...
    parser = GbBNewsParse("https://gb.ru/posts", db)
    parser.run()
