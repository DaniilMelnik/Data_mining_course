...
class Parse5ka:
    ...
    def _parse(self, url: str):
        while url:
            ...
            url = data["next"]
            if url:
                url = url.replace(urlparse(url).netloc, urlparse(self.star_url).netloc)
...
class ParseCategories(Parse5ka):
    def __init__(self, start_url: str, save_path: Path):
        ...
        self.product_url = "https://5ka.ru/api/v2/special_offers/"

    def run(self):
        response = self._get_response(self.star_url, headers=self.headers, params=self.params)
        for category in response.json():
            self.params.update(categories=category["parent_group_code"])
            data = {
                "name": category["parent_group_name"],
                "code": category["parent_group_code"],
                "products": [product for product in self._parse(self.product_url)],
            }
...
if __name__ == "__main__":
    ...
    cat_save_path = get_save_path("categories")
    cat_url = "https://5ka.ru/api/v2/categories/"
    cat_parser = ParseCategories(cat_url, cat_save_path)
    cat_parser.run()
